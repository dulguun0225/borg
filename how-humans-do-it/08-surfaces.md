...
- **Ops** — deployed software per environment: which release of each service is running, what contracts it publishes, health, incidents, in-flight rollouts and the control each is measured against. An acting surface, not watch-only: exercise veto after the fact (10) — rolling back while a control stands, raising the revert after — and page.
