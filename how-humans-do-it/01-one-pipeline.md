...
**Queue order.** Waiting in the merge queue is one place a queue forms, and its order is settable — an urgent item goes to the front. That is not a bypass. Reordering a queue changes when an item reaches the gates; it does not change which gates it passes through. Numbered releases are the other queue, and it forms only where a service already has its K windows open or a rollback is holding its deploys. That one's order is not settable: the number orders builds, so they leave for production in it.
