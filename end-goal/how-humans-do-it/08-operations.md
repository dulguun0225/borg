...
A rollout fails when the comparison crosses the boundary, and the rollback follows on its own. The comparison runs after every production deploy, and the strategy decides only how the factory acts on it: a rollout with a control sheds its traffic back onto the build still serving, whatever schedule it was taking that traffic on, and a straight deploy, having nothing standing to cut to, has to put a build back and wait for it.
...
The **watch window** is how long the factory may act on the comparison alone. Inside it, crossing the boundary rolls the release back with no human in the loop; outside it, the same crossing raises an item. It is the factory's own authority and is bounded by evidence, where veto after the fact (10) is a human's and is bounded by reversibility — a rollback while the control stands, a revert once it is gone. That is why there is no second named window: what bounds a human's undo is the control, which the strategy either paid for or did not, and a window with no parameter and no bound of its own is a name over a thing already named. A rollback deploy opens no window of its own: the build coming back was the control, and a fresh window would compare it against the release just condemned and put that one back.
...
| **swept** | a rollback aimed below it undid this release | closed with the release neither condemned nor standing |
...
Closing swept does not, and that is what separates the two unresolved exits. A release a rollback took with it was never condemned either — its comparison simply stopped, because master is linear and it sat above the target — but it is not running and its control went when its window did, so there is nothing there to go back to. The target the rollback aimed at is what stands afterwards and stays the target. A bystander is a release the factory reached past, not one it can reach for.
...
The factory works the item it raised under the attempt bound like any other. Hitting that bound turns it into an escalation (12), the same Work row a stuck feature produces: a bug the factory cannot fix is not a different kind of stuck.
