...
- **Factory** — the machine itself. Gate and risk policy, thresholds, strategy pins, environments, the [_agent fleet_](10-fleet.md) and the credential each entry stands on — and the same screen carries the readout: throughput, rework rate, gate rejection rate, cost per feature, what each agent is doing and how well. Cost per feature is the one readout whose control is not the factory's — what bounds it is the quota on the provider account each entry names, which [_The fleet_](10-fleet.md#what-the-fleet-is-not) declines to put a ceiling of its own over, and it is why the number and the entry sit on one screen. Not stage definition: the stages are the factory's own.
