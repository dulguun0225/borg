...
force](../02-intent-into-items/01-intake/01-constraints-and-the-design-system.md), the pass
over [the advisory feed](../02-intent-into-items/01-intake/03-advisories.md), and the pass over [the deprecation list](../07-contracts/08-deprecation.md). [Dispatch](../02-intent-into-items/05-dispatch.md)'s one for the pass that argues a [fleet proposal](../10-fleet/07-a-fleet-proposal.md) is the same shape, so a stopped pass, a raise or the factory's own arguing alike, is a named row rather than clean numbers. A last check is the writing component's own
...
A third comparison reads the last check records themselves, and it is what makes a stopped factory component reach a human. The detector already reads its own record's interval to say when a pass is owed, and the same reading over the others costs it nothing new. A last check past the interval it names with a further pass owed is a mismatch of the shape the two above have, holding what the stopped component reaches and paging: the health monitor's holds that service's production deploys, the deployer's holds that environment's. It stays inside the rule rather than widening it, because a factory component having stopped is not evidence about the software, and the detector still cannot cause anything to be built, deployed, scored, approved, or measured. What it reaches that nothing else could is the fourth [page](07-pages.md) condition, a window past its cap that nothing has evaluated. The health monitor is the only thing that closes a window, so the component that would have raised that page is the one that stopped, and the reader has to be outside the factory. The mismatch a stopped raise makes holds nothing, the two passes reaching no deploy, and the page is the whole of it.

Two stops that comparison finds have no carrier inside the factory: the notifier's own, for the reason [_Pages_](07-pages.md) gives, a mismatch about the notifier being delivered by the notifier; and the whole process's, every last check stale at once and the home view that would show them served by the process that stopped. For those two the detector carries one delivery of its own. Installing it includes writing one address into its store, mail or chat, the owner's and nobody's duty, and when the third comparison finds the notifier's last check stale or every factory last check stale at once the detector delivers to that address itself, naming what it found, and records the delivery in its own store. 

It is a page in every sense but the log's: the log is inside the stopped process, so the page event is appended when the factory next starts, [the notifier](07-pages.md) reading the detector's store as it already does for a cleared mismatch and writing the event with the detector's time on it. A heartbeat to a service outside the install that pages on its absence is refused: it makes a page depend on a service the customer runs or subscribes to beside the factory, which the self-hosted shape has so far refused, where the detector is already a second process the owner runs. What it costs is one address kept in a store [_People_](../11-screens/01-work-ops-factory-people.md) does not hold, a page the log holds late and the [page channel's numbers](../11-screens/05-the-page-channel-and-what-reached-a-human.md) count late, a second delivery path to keep working, and a detector whose stopping now matters. So the notifier pages whoever installed the detector when the detector's own last check is past the interval it names, the two watching each other and neither watching itself. What the comparison costs is a page for a component whose pass merely ran late, which is the [home view](../11-screens/02-three-properties-every-screen-needs.md)'s stated cost arriving on the narrow channel, bounded by the interval each record carries rather than by anything an owner authors.
...
The page is on the mismatch, one per mismatch however many firings the hold stops, and reaches whoever installed the detector as _Pages_ routes it. The production deploy row it holds pages nobody, as [_Deploy to production_](../03-gates/07-what-particular-gates-decide/08-deploy-to-production.md) states of that hold, so a firing that finds a mismatch and a revert decision standing at once sends the revert decision's page and no second. What it costs is a page naming no decision: which rows the mismatch holds is read at Ops and not on the page.
