...
One question gates the milestone rather than a task inside it: [_Open_](end-goal/open.md) holds how a verdict joins the chain, and M1's one gate is exactly the two-write decision that question is about — opened at the firing so the human has the [factor vector](end-goal/how-humans-do-it/04-risk-score.md#factors-at-least) to argue with, closed at the verdict. M0 appended whole records and never met it. Everything below can be built while it is open; the gate cannot, and it is the owner's to decide.
