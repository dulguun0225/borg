...
- Step 6, Traffic-shifting rollout and held-out control. No split. Directories changed: `factory/cmd/factory`, `factory/deploy` (**doc.go changed**), `factory/gate`, `factory/healthmonitor` (**doc.go changed**), `factory/localtarget` (**doc.go changed**), `factory/score` (**doc.go changed**), and `HANDOFF.md`, plus the five claims flipped to `built` in `end-goal/claims.txt`. The deploy package owns production control and kept fleets, including the customer's running build at adoption, builds reaches from the score pick's share, and consumes the health monitor's rollback target rather than selecting `Current`. The health monitor opens adoption's two-build comparison and the queue releases an awaited revert from only the rollback and window holds, preserving budget, dependency, drift, and human holds. Localtarget runs the release beside the already-running control/kept process from a shared traffic file; staged and full shifts reuse that identity, and rollback shifts back to the warm kept fleet. The held-out command test runs against localtarget with no adapter and verifies the control release/build, fleet counts, window state, and traffic file. Checks: `go vet ./...` passed; `go run ./cmd/depscheck` passed; `go run ./cmd/tracecheck` passed; `go test -count=1 ./localtarget ./deploy ./healthmonitor` passed (`ok github.com/dulguun0225/borg/factory/localtarget 6.627s`, `ok github.com/dulguun0225/borg/factory/deploy 2.360s`, `ok github.com/dulguun0225/borg/factory/healthmonitor 9.076s`); `go test -count=1 -timeout 60m ./cmd/factory -run 'Control|Traffic|HeldOut|Rollback|Adopt'` passed (`ok github.com/dulguun0225/borg/factory/cmd/factory 313.321s`); `git diff --check` passed.
- Step 7, rollback ordering, hosting-hour learning, target removals, operational holds, and accepted-commit re-verification. No split. Directories changed: `factory/cmd/factory` (**doc.go changed**), `factory/deploy` (**doc.go changed**), `factory/environment` (**doc.go changed**), `factory/healthmonitor` (**doc.go changed**), `factory/mergequeue` (**doc.go changed**), `factory/notifier` (**doc.go changed**), `factory/score` (**doc.go changed**), `factory/service` (**doc.go changed**), and `HANDOFF.md`, plus the claims flipped to `built` in `end-goal/claims.txt`. The score learning pass now receives composed readers for recorded environment-hours per item and instance-hours per release; controlled rollout share is separate from band width and lowers with hazard severity; healthmonitor records and pages a failed target-unreachable rollback before closing its window; service target writes determine removed addresses and refuse while the composed deploy-removal reader says removal has not happened, and deploy exposes the one-target removal; accepted commits route through queue re-verification and security predicates; the mergequeue C2065 citation was removed. The follow-up corrected first target authoring so an empty prior field is not treated as a removal, and the held-out control assertion follows the recorded picked share after hazard adjustment. Checks: `go vet ./...` passed; `go run ./cmd/depscheck` passed; `go run ./cmd/tracecheck` passed; `go test -count=1 ./score ./healthmonitor ./mergequeue ./service ./deploy ./environment` passed (`ok github.com/dulguun0225/borg/factory/score 3.363s`, `ok github.com/dulguun0225/borg/factory/healthmonitor 11.720s`, `ok github.com/dulguun0225/borg/factory/mergequeue 15.083s`, `ok github.com/dulguun0225/borg/factory/service 2.837s`, `ok github.com/dulguun0225/borg/factory/deploy 3.133s`, `ok github.com/dulguun0225/borg/factory/environment 8.384s`); `go test -count=1 -timeout 60m ./cmd/factory -run 'Rollback|Window|Kept|Credential|Hours|Learn|Remove|Retire'` passed (`ok github.com/dulguun0225/borg/factory/cmd/factory 442.217s`); follow-up `go vet ./...` passed; `go run ./cmd/tracecheck` passed; `go test -count=1 ./cmd/driftdetector ./service ./deploy` passed (`ok github.com/dulguun0225/borg/factory/cmd/driftdetector 7.072s`, `ok github.com/dulguun0225/borg/factory/service 2.469s`, `ok github.com/dulguun0225/borg/factory/deploy 2.725s`); and `go test -count=1 -timeout 60m ./cmd/factory -run 'HeldOut|Control|Rollback'` passed (`ok github.com/dulguun0225/borg/factory/cmd/factory 327.210s`). `git diff --check` passed; all changed source and test files are under 500 lines.
