...
- Step 4, Encoding and mutation at the candidate run. No split. Directories changed: `factory/agent` (**doc.go changed**), `factory/buildrunner` (**doc.go changed**), `factory/cmd/driftdetector`, `factory/cmd/factory`, `factory/contractcheck` (**doc.go changed**), `factory/criterion` (**doc.go changed**), `factory/deploy` (**doc.go changed**), `factory/gate` (**doc.go changed**), `factory/localtarget` (**doc.go changed**), `factory/securitypredicate` (**doc.go changed**), and `factory/targetseam` (**doc.go changed**), plus `HANDOFF.md`, `factory/deps.txt`, and the seven claims flipped to `built` in `end-goal/claims.txt`. `buildrunner` compiles candidate-artifact mutants without records using four bounded operators—negate a condition, flip a comparison, replace a literal, and drop a statement—over diff-touched non-test, non-import lines; `deploy` owns the serialized restore/place/run/environment-hour loop and records the score through `criterion`; `targetseam` and `localtarget`/`Fake` place the transient artifact without a deploy record and restore the selected seed between mutant runs where a seed exists, while unseeded candidates run without a restore-capable target; `contractcheck` owns the C1599 store check; the security decision carries the candidate run identity and the shipped Go list holds no predicates, as the owner's question in `end-goal/open.md` leaves that content unspecified. The required `TestMergeToMasterRejectsCandidateOnAnUncaughtMutant` proves Merge to master rejects a candidate on a mutant its criteria did not catch. C1456 now keeps each coverage row's own reason and preserves digestless resolution, C1459 records resolver enumeration rather than directory presence, and C1446 remains exposure's claim. The duplicate mutation write was caused by re-verification creating a new build while the mutation path used the candidate's older `c.buildID` to choose the run number and record the score; the actual build ID now flows through the criteria path, so each build/run receives one mutation row. The fixtures author mutant cap 1, with the mutation rejection fixture raising it to 2, and the target seam restores the seed before each mutant. Checks: `go vet ./...` passed; `go run ./cmd/depscheck` passed; `go run ./cmd/tracecheck` passed; `go test -count=1 ./localtarget ./targetseam ./deploy ./criterion ./mergequeue` passed; `go test -count=1 -timeout 60m ./cmd/factory -run 'TestTwoCandidatesProceedAtOnce|TestTheRollbackHoldsUntilTheRevertShips|Mutation|Mutant'` passed (`ok  github.com/dulguun0225/borg/factory/cmd/factory 175.579s`); `git diff --check` passed. Latest no-store checks: `go vet ./...`, `go run ./cmd/depscheck`, `go run ./cmd/tracecheck`, `go test -count=1 ./deploy ./localtarget ./targetseam`, and `go test -count=1 -timeout 60m ./cmd/factory -run 'TestAReportArrives|TestReportsBecomeIntents|Mutation'` passed.
...
Read `CLAUDE.md`, this handoff, the named M10 step-4 design files, `factory/README.md`, `factory/deps.txt`, and the affected package `doc.go` files before coding. Steps 1–4 are implemented; step 4 was not split. The corrected focused checks and unresolved design choices are recorded above.
