...
**A consumer declares what it assumes, and the candidate is checked against it.** Alongside the mode: which fields it reads, that they arrive populated, in what units, inside what domain. The factory already knows who consumes what, so the candidate standing in the UAT slot is checked against every consumer's declaration, and a failure is a rejection at the merge gate like any breaking diff. No window bounds it — a consumer called once a month is covered exactly as well as one on a hot path. This is the layer that is bought rather than had: the declarations are authored work per contract, taken where the contract is worth it.
...
### What may a consumer declare, and when does a declaration go stale?

A declaration is only cheap while it is mechanical. If a consumer may assert anything it likes in prose, checking a candidate against it stops being a diff and becomes an adjudication at the producer's merge gate — the per-change consumer veto _Who owns a contract_ rejected, arriving by another door. Where the line falls between an assumption the factory can check and a wish only a human can read is not settled.

Staleness is the same mechanism from the other end. A declaration outlives the code that motivated it: a consumer that declares it reads a field and then stops reading it pins that field in place, the list _Deprecation_ waits on never empties, and the removal item is never raised. What re-derives a declaration, and on whose item, is open with it.
