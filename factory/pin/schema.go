...
// columns are empty unless it is a predicate, and a pin that adds a human carries
// none of them. The CHECK refuses more than one at once, which is as far as the
// store can go — which shape a parameter takes is package gatepolicy's table, and
...
var DDL = []string{
	...
	constraint subject_kind_known check (subject_kind in ('service', 'area', 'gate_row',
		'factory_policy', 'contract_element')),
