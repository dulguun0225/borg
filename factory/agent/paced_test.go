...
func (m *countingModel) Complete(_ context.Context, _ principal.Principal, _ Call) (Reply, error) {
...
func TestPacedLeavesTheIntervalBetweenCalls(t *testing.T) {
	...
	start := time.Now()
	for n := range 3 {
		if _, err := paced.Complete(context.Background(), as(), Call{System: "s", User: "u"}); err != nil {
...
func TestPacedSendsNothingOnceTheContextIsCancelled(t *testing.T) {
	...
	cancel()
	_, err := paced.Complete(ctx, as(), Call{System: "s", User: "u"})
