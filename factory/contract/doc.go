...
// # The files
//
// contract.go is [Contract], [Version], and [ElementSubject]. form.go is [Kind]
// with [Kinds] and [Kind.Forward], [Position] with [Positions], [ElementKind]
// with [ElementKinds], [Range], [Element], [Form] with [Form.Validate],
...
// [Table], [VersionTable], [ElementTable], the three id prefixes, and [DDL].
//
// db_test.go is the tests against the database; derive_test.go is the
// derivation and diff_test.go the diff, neither of which needs one.
//
...
//
// An [Element] is one field, one operation, one argument or one message of the
// form, and [Element.Position] is which way compatibility runs for it. [Diff]
// applies the position's rule: in output position an addition is compatible and a
// removal or a redeclaration breaks, in input position a required addition, a
// narrowed range, a withdrawn value and a dropped argument break where an optional
// addition and a widened range do not, and a store breaks on the union of the two
// with the constraints it carries.
//
...
// be a component with one caller. A publication for a release naming no item is
// written: the queue mints such a release over an accepted commit, and the
// release is the key there.
//
...
// and [DeriveFile] is the one file package consumercontract derives a consumer's
// mirror through. How much of a build is visible is a property of its toolchain,
// so derive.go is Go's and a second toolchain replaces it rather than extending
// it; the convention, the two file names it reads, the struct tag, the directive
// on an operation, and what it cannot see are stated there. [Diff] is what one
// form does to the one before it, with the elements whose change the promise does
// not allow already computed from the position and the kind. Who is affected is
// not answered here: that is a query over the consumer contracts. [Get],
// [ByName], [OfService], [All], [VersionsOf], [NewestVersion], [ElementsOf], and
// [FormOf] are the reads.
//
