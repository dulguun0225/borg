// Package factorypolicy owns the factory policy record: the parameters an
...
// it limits is the factory's own and reaches every project at once. The
// attempt bound is per stage and the stages are the factory's; the predicate
// catalog is one list the factory owns and an owner extends; and the threshold
// the brief-or-skill gate row reads is a field of it because that row has no
// project and so no production environment to read. The record exists before
// any project does and an owner may never open it: where a field is unauthored
// the value in force is what the score supplies.
//
...
// authoring calls — [SetAttemptBound] and [SetPredicateCatalog], with
// [SetBriefOrSkillThreshold] beside them — take a transaction and are called by
...
// on production's environment record; the bound itself is
// ../../end-goal/how-humans-do-it/03-gates.md#the-attempt-bound.
