...
// [NewID] returns, the format version the table's own DDL constant supplies,
// the three actor fields, and the time the writer wrote the row in
// [TimeLayout]. A table composes it as the first thing inside its CREATE
// TABLE and follows it with a comma and its own columns.
//
// The timestamp is text and not timestamptz because a record whose hash
// covers it has to read back byte for byte, and a column type that parses and
// reformats what it stores cannot promise that.
const Columns = `id text not null primary key,
	format_version text not null,
	actor_kind text not null,
	actor_key text not null,
	actor_key_basis text not null,
	at text not null`
...
// within the schema, so every record table names these five the same and a
