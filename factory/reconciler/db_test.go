// The database tests of this package are in reconciler_test rather than in
// reconciler, the way every record package's are — except here it changes
// nothing in deps.txt: this store is not the factory's, so these tests open it
// through [reconciler.Open] and [reconciler.Apply] rather than through package
// postgres, and the only package this file imports besides reconciler itself
// is record, which reconciler's own line already allows.
//
...
func newTable(t *testing.T) (context.Context, *pgxpool.Pool, *reconciler.Writer) {
	t.Helper()
	ctx := t.Context()
	...
	pool, err := reconciler.Open(ctx, inSchema(t, reconciler.DefaultURL, schema))
	if err != nil {
		t.Fatalf("the database at %s is not reachable, and these tests do not skip: %v", reconciler.DefaultURL, err)
	}
	t.Cleanup(func() {
	...
	return ctx, pool, reconciler.NewWriter(pool)
}
...
func pass() reconciler.Pass {
	return reconciler.Pass{
...
func TestAPassWhereTheTargetAgreesWritesNoMismatch(t *testing.T) {
	...
	if !recorded.Comparison.Agreed {
		t.Error("Comparison.Agreed = false, want true where the target runs the recorded build")
	}
	if recorded.Raised != "" || recorded.Agreed != "" {
		t.Errorf("Record = %+v, want neither a raise nor an agreement recorded", recorded)
	}
	held, why, err := reconciler.NewStore(pool).Mismatch(ctx, p.ServiceID)
	if err != nil || held {
...
// mismatch a pass writes: raised, found by [reconciler.Uncleared], and read
// back through [reconciler.Store.Mismatch] the way the production deploy
// gate does.
func TestAPassWhereTheTargetDisagreesRaisesAMismatchTheGateReads(t *testing.T) {
	...
	uncleared, err := reconciler.Uncleared(ctx, pool, p.ServiceID)
	...
	for _, want := range []string{reconciler.HoldWords, p.Target, p.RunningBuild, p.RecordedBuildID} {
...
func TestALaterAgreeingPassRecordsAnAgreementAndLeavesTheMismatchStanding(t *testing.T) {
	...
	standing, err := reconciler.Get(ctx, pool, first.Raised)
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	...
	held, _, err := reconciler.NewStore(pool).Mismatch(ctx, p.ServiceID)
	if err != nil || !held {
		t.Errorf("Mismatch = %v, %v, want true — a mismatch remains even where a later comparison agrees", held, err)
	}
}

func TestAnUnreachedTargetWritesTheLastComparisonAndRaisesNoMismatch(t *testing.T) {
	ctx, pool, w := newTable(t)
	p := pass()
	p.Reached = false
	...
	if recorded.Comparison.Reached || recorded.Comparison.Why != p.Why {
		t.Errorf("Comparison = %+v, want unreached with why %q", recorded.Comparison, p.Why)
	}
	if recorded.Raised != "" || recorded.Agreed != "" {
...
func TestAPassMissingSomethingEveryComparisonNamesIsIncomplete(t *testing.T) {
	...
	for _, c := range []struct {
		what string
		mut  func(*reconciler.Pass)
	}{
		{"service", func(p *reconciler.Pass) { p.ServiceID = "" }},
		{"target", func(p *reconciler.Pass) { p.Target = "" }},
		{"why on an unreached target", func(p *reconciler.Pass) { p.Reached = false; p.Why = "" }},
	} {
		p := pass()
		c.mut(&p)
		if _, err := w.Record(ctx, p); !errors.Is(err, reconciler.ErrPassIncomplete) {
			t.Errorf("Record missing %s = %v, want ErrPassIncomplete", c.what, err)
		}
	}
}

func TestTheLastComparisonIsOverwrittenNotAppended(t *testing.T) {
	ctx, pool, w := newTable(t)
	p := pass()
	...
	comparisons, err := reconciler.LastComparisons(ctx, pool, p.ServiceID)
	if err != nil {
		t.Fatalf("LastComparisons: %v", err)
	}
	if len(comparisons) != 1 {
		t.Fatalf("LastComparisons = %+v, want exactly one row", comparisons)
	}
	if comparisons[0].RunningBuild != second.RunningBuild {
		t.Errorf("the last comparison runs %q, want the second pass's %q", comparisons[0].RunningBuild, second.RunningBuild)
	}
}
...
func TestClearMarksAMismatchClearedAndKeepsTheRow(t *testing.T) {
	...
	all, err := reconciler.All(ctx, pool)
	if err != nil || len(all) != 1 || all[0].ID != raised.Raised || !all[0].Cleared() {
		t.Errorf("All = %+v, %v, want the cleared row kept", all, err)
	}

	if _, err := w.Clear(ctx, raised.Raised, "bob"); !errors.Is(err, reconciler.ErrAlreadyCleared) {
...
func TestClearWithNoHumanIsClearedByEmpty(t *testing.T) {
	...
	if _, err := w.Clear(ctx, raised.Raised, ""); !errors.Is(err, reconciler.ErrClearedByEmpty) {
...
func TestGetOnAnUnknownIDIsNotFound(t *testing.T) {
	...
	if _, err := reconciler.Get(ctx, pool, "mis_00000000000000000000000000000000"); !errors.Is(err, reconciler.ErrNotFound) {
...
// the exception for a build an open window accounts for: [reconciler.Pass.Excused]
...
func TestAnExcusedPassAgreesDespiteADifferentRunningBuildAndRaisesNoMismatch(t *testing.T) {
	...
	if !recorded.Comparison.Agreed {
		t.Error("Comparison.Agreed = false, want true — an open window's exception makes this no disagreement")
	}
	if recorded.Raised != "" {
...
// TestDDLRefusesAMismatchClearedWithoutAHumanAndAComparisonUnreachedWithNoWhy
...
// exits.
func TestDDLRefusesAMismatchClearedWithoutAHumanAndAComparisonUnreachedWithNoWhy(t *testing.T) {
	ctx, pool, _ := newTable(t)

	_, err := pool.Exec(ctx, `insert into `+reconciler.MismatchTable+`
		...
		values ($1, 'component', 'reconciler', $2, $3, $4, $5, $6, $7, 0, $8, '')`,
		record.NewID(reconciler.MismatchIDPrefix), record.Now(), record.NewID("svc"), record.NewID("tgt"),
	...
	_, err = pool.Exec(ctx, `insert into `+reconciler.ComparisonTable+`
		(id, actor_kind, actor_name, at, service_id, target, reached, why,
		 running_build, recorded_release_id, recorded_build_id, agreed)
		values ($1, 'component', 'reconciler', $2, $3, $4, false, '', $5, $6, $7, false)`,
		record.NewID(reconciler.ComparisonIDPrefix), record.Now(), record.NewID("svc"), record.NewID("tgt"),
		record.NewID("bl"), record.NewID("rel"), record.NewID("bl"))
	if err == nil {
		t.Error("the store accepted a last comparison unreached with no reason")
	}
}
