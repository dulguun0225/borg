...
// MismatchTable and ComparisonTable are the two tables this package owns, in a
// store of its own.
const (
	MismatchTable   = "reconciler_mismatch"
	ComparisonTable = "reconciler_last_comparison"
)

// The prefixes [record.NewID] is called with for each.
const (
	MismatchIDPrefix   = "mis"
	ComparisonIDPrefix = "rcm"
)

// DefaultURL is the reconciler's own store as the demonstration runs it: the
...
const DefaultURL = "postgres://factory:factory@localhost:5433/factory?search_path=reconciler"

// URLEnv is the environment variable that names the reconciler's store. It is a
// variable of its own and not the factory's, so pointing the reconciler somewhere
// else is one setting and not a change to the factory's configuration.
const URLEnv = "RECONCILER_DATABASE_URL"
...
func Open(ctx context.Context, url string) (*pgxpool.Pool, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reconciler: opening the pool: %w", err)
	}
	if err := pool.Ping(ctx); err != nil {
		pool.Close()
		return nil, fmt.Errorf("reconciler: reaching its store: %w", err)
	}
	return pool, nil
}

// Apply creates the reconciler's schema. It is called by the reconciler's own
// process and by nothing in the factory.
func Apply(ctx context.Context, pool *pgxpool.Pool) error {
	for n, statement := range DDL {
		if _, err := pool.Exec(ctx, statement); err != nil {
			return fmt.Errorf("reconciler: applying statement %d: %w", n+1, err)
		}
	}
	return nil
}
...
//
// The last comparison is one row per service and target, which the unique
// constraint enforces and [Writer.Record]'s upsert conflicts on: it is overwritten
// each pass, because what it answers is whether the reconciler is still running and
// not what it has ever said.
var DDL = []string{
