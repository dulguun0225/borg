...
// Actor is who the reconciler's rows are written as. It is a component like any
// other: what makes this store independent is that no factory component may write
// it, not that its rows have no author.
var Actor = record.Actor{Kind: record.KindComponent, Name: "reconciler"}
...
var (
	...
	ErrPassIncomplete = errors.New("reconciler: the pass is missing something every comparison names")
	// ErrNotFound is returned where no mismatch has that id.
	ErrNotFound = errors.New("reconciler: no mismatch has that id")
	...
	ErrAlreadyCleared = errors.New("reconciler: the mismatch is cleared already")
	// ErrClearedByEmpty is returned by [Writer.Clear] naming no human. Clearing is a
	// human's act at the reconciler and the record says whose.
	ErrClearedByEmpty = errors.New("reconciler: a mismatch is cleared by a named human")
)
...
type Mismatch struct {
	...
	// reconciler read. The target is named rather than the environment, which is the
	// stronger check — a deploy record per target would let three targets disagree
	// with a fourth and call each of them right.
	ServiceID string
	Target    string
...
const HoldWords = "the reconciler found a record disagreeing with what runs"

// Comparison is the last comparison of one service on one target, overwritten each
// pass.
...
// Pass is one comparison the reconciler performed, as its caller hands it over.
type Pass struct {
	ServiceID string
	Target    string
	...
	// unreached pass writes the last comparison and nothing else.
	Reached bool
...
// Recorded is what one pass wrote: the last comparison always, and one of a new
...
// Writer is the one writer of both records: the reconciler's own process. No
...
// Record writes what one pass found: the last comparison, overwritten; a mismatch
...
//
// The last comparison is written first and unconditionally. It is the record that
// says the reconciler ran, and a pass that failed to write a mismatch afterwards is
// still a pass that happened — where the other order would leave a stopped reconciler
// and a raised mismatch indistinguishable from a reconciler that never ran.
func (w *Writer) Record(ctx context.Context, p Pass) (Recorded, error) {
	...
	c := Comparison{
		ID:                record.NewID(ComparisonIDPrefix),
		Actor:             Actor,
		At:                record.Now(),
	...
	_, err := w.pool.Exec(ctx, `insert into `+ComparisonTable+`
	...
	if err != nil {
		return Recorded{}, fmt.Errorf("reconciler: recording the comparison of %s on %s: %w",
			p.ServiceID, p.Target, err)
	}
	recorded := Recorded{Comparison: c}
	...
	case agreed && found:
		if _, err := w.pool.Exec(ctx, `update `+MismatchTable+`
			...
			return recorded, fmt.Errorf("reconciler: recording a later agreement on %s: %w", standing.ID, err)
		}
	...
	case !agreed && !found:
		...
		if err != nil {
			return recorded, fmt.Errorf("reconciler: raising a mismatch on %s: %w", p.Target, err)
		}
...
// Clear is a human at the reconciler clearing one mismatch. It is here and there is
// no counterpart in the factory: clearing it from Ops would make the factory a writer
// of the record that says the factory is wrong.
//
...
func (w *Writer) Clear(ctx context.Context, id, by string) (Mismatch, error) {
	...
	tag, err := w.pool.Exec(ctx, `update `+MismatchTable+`
		...
		record.Now(), by, id)
	if err != nil {
		return Mismatch{}, fmt.Errorf("reconciler: clearing %s: %w", id, err)
	}
