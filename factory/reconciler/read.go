...
// [NewWriter] is the reconciler's own.
...
func Get(ctx context.Context, pool *pgxpool.Pool, id string) (Mismatch, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		return Mismatch{}, fmt.Errorf("reconciler: reading %s: %w", id, err)
	}
	return m, nil
}
...
func mismatches(ctx context.Context, pool *pgxpool.Pool, suffix string, args ...any) ([]Mismatch, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reconciler: reading the mismatches: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("reconciler: reading a mismatch: %w", err)
		}
		read = append(read, m)
	}
...
func unclearedOn(ctx context.Context, pool *pgxpool.Pool, serviceID, target string) (Mismatch, bool, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		return Mismatch{}, false, fmt.Errorf("reconciler: reading the mismatch on %s: %w", target, err)
	}
	return m, true, nil
}

// LastComparisons is the last comparison of every target of one service, or of
// every target where the service is empty. It is what says whether the reconciler is
// still running: a check that silently stops is worse than the bug it catches, so this
// is read rather than the absence of mismatches being taken as health.
func LastComparisons(ctx context.Context, pool *pgxpool.Pool, serviceID string) ([]Comparison, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("reconciler: reading the last comparisons: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &kind, &c.Actor.Name, &c.At, &c.ServiceID, &c.Target,
			...
			return nil, fmt.Errorf("reconciler: reading a last comparison: %w", err)
		}
