...
func (h *HealthMonitor) Open(ctx context.Context, w Watching, deployID, releaseID, scoreVersion string, heldOut bool) (window.Window, bool, error) {
	...
	below, err := release.Below(ctx, h.pool, w.ID, rel.Number)
	if err != nil {
		return window.Window{}, false, err
	}
...
// what the window limit is. An open window blocks nothing until as many as the limit
// allows are open, and
// then the next production deploy holds — a wait on the factory, which does not
// page, so it shows only to a reader who asks.
//
// It is here rather than in whatever computes the hold, because the limit in force and
