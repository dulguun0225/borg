...
// window closed without failing it — passed or timed out — and false where
...
//     would return a release above the failed one and the factory would restore
//     the change it had just failed;
...
//
// It descends past failed, past skipped, and past any window still open. Nothing
...
// rollback goes further back and undoes releases nothing failed, up to the window
// limit, which
...
func TargetBelow(ctx context.Context, pool *pgxpool.Pool, serviceID string, number int64) (release.Release, bool, error) {
	closed, err := window.ClosedWithoutFailing(ctx, pool, serviceID)
	if err != nil {
