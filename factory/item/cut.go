...
// ErrAlreadySuperseded is returned by [Cut.Supersede] for an item that is
...
// ErrMerged is returned by [Cut.Supersede] for a merged item. A merged item is
// out of reach of a re-cut — a return may be raised no later than the merge to
// master — so a re-cut leaves shipped work alone and declares the new set's order
// against it.
var ErrMerged = errors.New("item: a merged item is out of a re-cut's reach")

// Supersede ends one item because a cut replaced it: the stage becomes
// superseded and the item points at whatever replaced it, which is empty where a
// re-cut replaced it with nothing.
//
// It is the cut's second write and its only write to an existing item, which is
// the seam this package's doc.go states: the cut writes an item when it creates
...
// tell from one a re-cut dropped.
func (c *Cut) Supersede(ctx context.Context, actor record.Actor, itemID string, replacedBy []string) (Item, error) {
	if err := actor.Validate(); err != nil {
		return Item{}, err
	}
	if itemID == "" {
