...
is at the 500-line bound, and locality's answer to a file at the bound is to split by
subject rather than grow it. `factory-constraint.spec.ts` is this screen's one departure
from a spec per screen directory, named for the address whose tests it holds.
