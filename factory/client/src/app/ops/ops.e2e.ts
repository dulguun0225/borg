// What Ops is decided on in a browser and nowhere a spec fakes the factory:
// that the screen the binary serves reads the Ops address as the declared
// human and renders what the factory's own records hold.
//
// A run that has supplied no intent holds no service record, so the board is
// the screen's empty state, and what this decides is that the empty state is
// reached over the real read and not that a row renders.
//
...
test('Ops reads what is running and says the factory holds no service yet', async ({ page }) => {
  ...
  await expect(page.getByText('The factory holds no service on an environment yet.')).toBeVisible();
});
