...
// hands the four kinds below to the readers beside it. reach.go is the four
// readers over the diff — [outboundCall], [credential], [authorizationCheck] and
// [dependencyChange] — with the package list, the credential words and the
...
//   - A credential named or read: a new string literal shaped like a secret's
//     name, or a new call to secretref.Resolve or os.Getenv on a name carrying
...
//     around one.
//   - A dependency change: a changed line of go.mod or go.sum naming a package
//     and a version, with the licence the build's resolved set gives it where
//     the set names one.
//
