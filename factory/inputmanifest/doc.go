...
//
// A manifest names the item and its stage, the intent, or the project, the
// dispatch was for — the third being the grouper, the one role put on neither
// an item nor an intent; the material handed over, one entry per source with its class,
...
//
// item_id, intent_id and project_id are id fields and not foreign keys, like
// every link between records; record's doc.go states that rule and its cost
// once.
//
