...
func fixedRow() Row {
	return Row{
		...
		Actor:         record.Actor{Kind: record.KindComponent, Name: "gate.merge"},
...
// hash to move. A field the hash does not cover is a field that can be edited
// in the store without Verify noticing.
func TestChainHashCoversEveryField(t *testing.T) {
	...
	changes := map[string]func(*Row){
		...
		"Actor.Name":    func(r *Row) { r.Actor.Name = "owner" },
...
func TestChainHashCannotBeForgedAcrossFields(t *testing.T) {
	one := fixedRow()
	one.Actor.Name = "gate.merge"
	one.Payload = "x"

	other := fixedRow()
	other.Actor.Name = "gate.mergex"
	other.Payload = ""
...
func TestDDLListsEveryShape(t *testing.T) {
	...
	if len(listed) != len(Shapes) {
		t.Fatalf("the constraint lists %d shapes, Shapes has %d", len(listed), len(Shapes))
	}
