...
// area is for is being named by something else: a pin or a scope drawn on an
// area reaches every item in it, and the item-size target is per area.
//
// # The chain
//
// Areas form a chain, each naming the area it lies inside, and [Chain] walks
// one from an area to the outermost. A pin drawn on any area in that chain
// reaches an item in the narrowest, which is why the walk exists and why a
// mechanism a pin binds asks for the chain rather than for the one area the
// item names. The design has the chain end at the project the area lies
// inside; there is no project record until the surfaces are built, so a chain
...
// The cut reads an area and writes none.
//
// What defines it:
...
// ../../end-goal/how-humans-do-it/02-intent-into-items.md#the-cut and the row
