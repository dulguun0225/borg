...
const (
	// ActionCreated is a record Factory created: the factory policy record, or
	...
	// ActionPinned is an owner placing a pin.
...
func appendVersion(ctx context.Context, tx pgx.Tx, actor record.Actor, action Action,
	parameter gatepolicy.Parameter, subject Subject, pinID string) (Version, error) {
...
const selectVersion = `select id, actor_kind, actor_name, at, action, parameter,
	subject_kind, subject_id, qualifier, pin_id, supersedes
	from ` + Table
...
func scanVersion(row pgx.Row, named string) (Version, error) {
	...
	err := row.Scan(&v.ID, &kind, &v.Actor.Name, &v.At, &action, &parameter,
		&v.Subject.Kind, &v.Subject.ID, &v.Subject.Qualifier, &v.PinID, &v.Supersedes)
