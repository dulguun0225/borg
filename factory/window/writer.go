...
// an exit is [ErrAlreadyClosed]: the health monitor evaluates every exit, so a second
...
// recomputed from the numbers it was decided on. The skipped exit takes none: a
...
func (w *Writer) Close(ctx context.Context, id string, exit Exit, on boundary.Observed) (Window, error) {
	...
	if exit == ExitSkipped && on != (boundary.Observed{}) {
