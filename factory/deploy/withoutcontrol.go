...
// WithoutControl is the rollout without a control: it writes the deploy record
// started, puts the build on the target through the seam, and advances the record
// to complete. The credential crosses as a [secretref.Ref] — a name and never a
// value — and the target names both a service id, which the record stores, and a
// service name, which the target acts on.
//
