...
// carries.
//
// service_id and release_id are id fields and not foreign keys: each is
// another package's record, and a cross-package link is a field the link walk
// reads. The store checks each for being present and not for pointing at
...
var DDL = []string{
	...
	constraint environment_present check (environment <> ''),
