...
//
// # Seven rows, eight parameters, and one that is pinnable only
//
...
//
// [PinOnly] is beside those eight and not among them: a pinned predicate is a pin
// on one element of a contract and nobody authors a value for it, so it belongs to
// the vocabulary of pins rather than to the rows of gate policy. Listing it with
// the eight would make the count of rows eight while changing nothing about what
// an owner may write, so it is a list of its own and [Define] reads both.
//
// # A pin can only add
//
...
// threshold's is the first: an owner
...
// catalog's floor is the second: a floor under a list is the union of the two,
// because a kind of assertion added is coverage added and one removed would
// invalidate declarations already ratified at a gate. A pinned predicate's is the
// third and it is the same shape as the catalog's one level down — the predicates
// in force on an element are the ones derived plus the ones pinned, and a pin adds
// to that set and takes nothing out of it. [PinOnly] says why that direction is
// derived rather than read off the design's own list.
//
// # The predicate kinds
//
// [PredicateKinds] is the other list this package holds, and it is vocabulary for
// the same reason the parameters are: package declaration derives a predicate,
// package policy resolves the catalog those kinds are the unauthored value of, and
// enforcement decides one. predicate.go says what each kind asserts, which of them
// can be decided against a form with no run to observe, and what a kind outside
// the five means.
//
// What defines it:
...
// for the scope of each and the direction its pin takes. The predicate kinds are
