...
// [Project] with [Project.Ended], [Writer] and [NewWriter] with [Writer.Create],
// the transaction-taking [Insert] and [End], and the reads [Get], [ByName] and
// [All]. The tests are db_test.go and end_test.go, every one of them against the
// database.
//
// The record holds its identity and when an owner ended it, and nothing an owner
// authors: what is authored per project is a field of production's environment
// record, of a constraint, of a safeguard, or of a fleet entry, each naming the
// project.
//
...
// own, which is what a test and the command-line interface use. [End] is the
// other end of the same pairing: a project is ended once every service in it is
// retired, and production's environment is withdrawn in the same transaction by
// the same caller. A project is never deleted, so an area, a constraint, a
// safeguard, or a scope naming one never points at nothing, and this package has
// no delete.
//
