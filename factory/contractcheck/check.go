...
type Checked struct {
	...
	Declares []consumercontract.Draft
	...
	// consumer contract to decide is a failure, and this is where a reader sees
	// which.
	Observed int
}
...
// consumer contract in force decided against the candidate's own run,
// and the candidate's own new consumer contracts decided against each producer's
// newest form.
//
...
func (c *Check) Enforce(ctx context.Context, candidate Candidate, production string) (Checked, error) {
	...
	// The consumers' consumer contracts, decided against the candidate's own run.
	// Only the ones naming a contract this candidate publishes: a predicate about
	// an interface this build does not publish is one this candidate cannot have
	// broken.
	...
	// newest release publishes. Only the kinds a form can decide: a domain and a
	// range are about values, and what decides those is the producer's own next
	// candidate.
...
func namingAnyOf(predicates []consumercontract.Predicate, forms []contract.Form) []consumercontract.Predicate {
	var naming []consumercontract.Predicate
	for _, p := range predicates {
