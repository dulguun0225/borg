...
type Checkout interface {
	...
	// catalog in force.
	Declares(ctx context.Context, c Candidate, catalog []string) ([]declaration.Draft, error)
}
...
// the comparison's signal is one — what emits it is the software the factory wrote
...
type Exchanges interface {
	...
	Observed(ctx context.Context, c Candidate) ([]declaration.Document, error)
}
...
var (
	...
	// had not been read.
	ErrNoExchanges = errors.New("contractcheck: a check with no run to observe cannot decide a consumer's declaration")
)

// Check is enforcement over one factory: the producer's own diff, every
// consumer's declaration, the declarations in force, the deprecation list, and the
// detector that raises a removal.
//
...
// intent in still enforces — the diff and the declarations are most of what
