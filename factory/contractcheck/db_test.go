...
// the two baselines, the range declarations in force are read over, the deprecation
// list emptying, and what a store's forward promise refuses. Each is a query over
// records a run writes in passing, so a run that got one wrong would fail somewhere
// else and say something else.
//
// The rejection at the merge row, the three items of a migration, and the pinned
// predicate are demonstrated through the crude interface in cmd/factory, where there
...
type fakeCheckout struct {
	...
	declares  map[string][]declaration.Draft
}
...
func (f *fakeCheckout) Declares(_ context.Context, c contractcheck.Candidate, _ []string) ([]declaration.Draft, error) {
	return f.declares[c.ItemID], nil
}

// fakeExchanges is the documents one build wrote, by build. No entry is no document
// observed, which enforcement treats as a failure wherever there is a declaration to
// decide.
type fakeExchanges struct {
	observed map[string][]declaration.Document
}

func (f *fakeExchanges) Observed(_ context.Context, c contractcheck.Candidate) ([]declaration.Document, error) {
...
func newGraph(t *testing.T) (context.Context, graph) {
	...
	g := graph{
		...
		exchanges: &fakeExchanges{observed: map[string][]declaration.Document{}},
	}
...
func draft(producer service.Service, interfaceName, element string,
	kind gatepolicy.PredicateKind, argument string) declaration.Draft {
...
// restore floor exists for.
func ship(t *testing.T, ctx context.Context, g graph, svc service.Service,
	forms []contract.Form, declares []declaration.Draft, exit window.Exit) (release.Release, string) {
	t.Helper()
	...
	if len(declares) > 0 {
		if _, _, err := g.store.SubmitDeclaration(ctx, theActor, theBy, it.ID, svc.ID,
			"derived from the build", declares); err != nil {
			t.Fatalf("submitting the declaration: %v", err)
		}
	}
	...
	w, err := g.windows.Open(ctx, record.Actor{Kind: record.KindComponent, Name: "comparison"}, window.Opening{
		DeployID: dep.ID, ReleaseID: rel.ID, ServiceID: svc.ID, CleanAvailable: true,
...
func candidateOf(t *testing.T, ctx context.Context, g graph, svc service.Service,
	forms []contract.Form, declares []declaration.Draft, documents []declaration.Document) contractcheck.Candidate {
	t.Helper()
	it, err := g.items.Create(ctx, theActor, item.New{
		IntentID: record.NewID("in"), ServiceID: svc.ID, Branch: "item/" + record.NewID("in"),
	})
	if err != nil {
		t.Fatalf("cutting the candidate's item: %v", err)
	}
	bl, err := g.builds.Create(ctx, theActor, it.ID, record.NewID("commit"))
	if err != nil {
...
func ok() []declaration.Document {
	return []declaration.Document{{"Status": "ok", "Detail": "fine"}}
}
...
func TestABreakingDiffIsRejectedWhereAConsumerStillDeclaresTheElement(t *testing.T) {
	...
	ship(t, ctx, g, g.producer, []contract.Form{full}, nil, window.ExitCap)
	ship(t, ctx, g, g.consumer, nil, []declaration.Draft{
...
// TestDeclarationsInForceRunFromTheRestoreFloorToTheNewest, and a service with no
// window closed clean or at the cap has no floor and every release it has is in the
// range.
func TestDeclarationsInForceRunFromTheRestoreFloorToTheNewest(t *testing.T) {
	ctx, g := newGraph(t)

	// Three releases of the consumer: the first two declare an element the third
	// stops declaring, and only the second's window closes at the cap. The floor is
	// the release the newest closed window watched, so releases 2 and 3 are in force
	// and release 1 is not.
	ship(t, ctx, g, g.consumer, nil, []declaration.Draft{
		draft(g.producer, theInterface, "Gone", gatepolicy.PredicateRead, ""),
	}, window.ExitHarm)
	second, secondWindow := ship(t, ctx, g, g.consumer, nil, []declaration.Draft{
		draft(g.producer, theInterface, "Detail", gatepolicy.PredicateRead, ""),
	}, window.ExitCap)
	ship(t, ctx, g, g.consumer, nil, []declaration.Draft{
		draft(g.producer, theInterface, "Status", gatepolicy.PredicateRead, ""),
	...
	in, err := g.check.DeclarationsInForce(ctx, g.consumer.ID)
	if err != nil {
		t.Fatalf("DeclarationsInForce: %v", err)
	}
	if !in.HasFloor || in.FloorNumber != second.Number || in.FloorWindowID != secondWindow {
		t.Fatalf("the floor is %+v, want release %d set by window %s", in, second.Number, secondWindow)
	}
	if in.HighestNumber != 3 {
		t.Errorf("the newest release is %d, want three", in.HighestNumber)
	}
	if len(in.ItemIDs) != 2 {
		t.Fatalf("the range holds %d items, want the two releases from the floor up", len(in.ItemIDs))
	}
	...
	if !named["Detail"] || !named["Status"] {
		t.Errorf("the declarations in force name %v, want Detail and Status", named)
	}
	if named["Gone"] {
		t.Error("a release below the restore floor is still in force, and a rollback cannot return to it")
	}
}

// TestAServiceWithNoClosedWindowHasNoFloorAndEveryReleaseInForce: which is the
// direction a first release's missing rollback target already takes.
func TestAServiceWithNoClosedWindowHasNoFloorAndEveryReleaseInForce(t *testing.T) {
	ctx, g := newGraph(t)
	...
	if in.HasFloor {
		t.Fatalf("a service with no closed window has a floor: %+v", in)
	}
	...
	// A service with no release at all has nothing in force: every declaration it
	// derived belongs to a candidate that has not merged.
	empty, err := g.check.DeclarationsInForce(ctx, g.producer.ID)
	if err != nil {
		t.Fatalf("DeclarationsInForce on a service with no release: %v", err)
	}
...
func TestTheDeprecationListIsAQueryAndTheDetectorRaisesTheRemovalOnce(t *testing.T) {
	...
	ship(t, ctx, g, g.producer, []contract.Form{marked}, nil, window.ExitCap)
	ship(t, ctx, g, g.consumer, nil, []declaration.Draft{
		draft(g.producer, theInterface, "Detail", gatepolicy.PredicateRead, ""),
	}, window.ExitCap)
...
// TestAPinnedPredicateBlocksTheRemovalAndIsToldApartFromADeclaration: a pin never
// stops the item existing, only passing, and what a reader of that rejection needs is
// the pin and its author.
func TestAPinnedPredicateBlocksTheRemovalAndIsToldApartFromADeclaration(t *testing.T) {
	ctx, g := newGraph(t)
	...
	placed, _, err := g.factory.Pin(ctx, theOwner, gatepolicy.PinnedPredicate,
	...
	// The detector still raises the removal — a pin never stops the item existing.
	raised, err := g.check.RaiseRemovals(ctx)
	if err != nil {
		t.Fatalf("RaiseRemovals: %v", err)
	}
	if len(raised) != 1 || !raised[0].New {
		t.Fatalf("the detector raised %+v with a pin standing, want the removal intent", raised)
	}
	...
	if checked.Passed() {
		t.Fatal("the removal passed with a pinned predicate naming the element")
	}
	if checked.Check() != gate.AutoRejectedByPinnedPredicate {
		t.Errorf("the check that rejected is %q, want the pinned predicate", checked.Check())
	}
	if !contains(checked.Why(), placed.ID) || !contains(checked.Why(), theOwner.Name) {
		t.Errorf("the rejection names neither the pin nor its author: %s", checked.Why())
	}

	// Withdrawing it lets the next candidate through, which is how an invented read
	// is taken back.
	if _, err := g.factory.WithdrawPin(ctx, theOwner, placed.ID); err != nil {
	...
	if !checked.Passed() {
		t.Fatalf("the removal is still refused after the pin was withdrawn: %s", checked.Why())
	}
}
...
func TestAStoresForwardPromiseRefusesAPopulatedAddition(t *testing.T) {
	...
	ship(t, ctx, g, g.producer, []contract.Form{stored(element("ID", "string", true, false))}, nil, window.ExitCap)
...
// TestAConsumersDeclarationIsDecidedAgainstTheCandidatesOwnRun: all five kinds are
// decidable against an observed exchange, and no exchange at all is a failure rather
// than a pass.
func TestAConsumersDeclarationIsDecidedAgainstTheCandidatesOwnRun(t *testing.T) {
	ctx, g := newGraph(t)
	...
	// declaration decided against the run.
	outside := candidateOf(t, ctx, g, g.producer, []contract.Form{full}, nil,
		[]declaration.Document{{"Status": "unknown"}})
	...
	if checked.Check() != gate.AutoRejectedByDeclaration {
		t.Errorf("the check that rejected is %q, want a consumer's declaration", checked.Check())
	}
	...
	if checked.Passed() {
		t.Fatal("a producer that wrote no exchange document passed a declaration it never showed holding")
	}

	// And one whose run stays inside it passes.
	inside := candidateOf(t, ctx, g, g.producer, []contract.Form{full}, nil,
		[]declaration.Document{{"Status": "ok"}, {"Status": "error"}})
...
// running, and a consumer's declaration against what its producer's newest release
// publishes.
func TestTheTwoBaselinesAreDifferent(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("cutting the merged item: %v", err)
	}
	...
	// removed on master fails at its own gate, because a declaration is checked
	// against the version its producer's newest release publishes.
	consuming := candidateOf(t, ctx, g, g.consumer, nil, []declaration.Draft{
		draft(g.producer, theInterface, "Detail", gatepolicy.PredicateRead, ""),
	}, nil)
	...
	if len(checked.Unmet) != 1 || checked.Unmet[0].Draft.Element != "Detail" {
		t.Fatalf("the unmet declarations are %+v", checked.Unmet)
	}
