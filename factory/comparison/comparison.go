...
// Actor is who the comparison's writes are made as: the watch window it opens and
// closes, the incident it raises, and the revert intent it takes in through intake.
var Actor = record.Actor{Kind: record.KindComponent, Name: "comparison"}
...
// human reads — and the production environment the comparison exists in and nowhere
// else.
...
// Signal is where the quantity comes from. It is an interface because what emits
// it is the software the factory wrote and where that lands is the substrate's
// arrangement: a comparison that read a file would be a comparison that only works
// on one kind of target.
type Signal interface {
	Read(ctx context.Context, q Quantity) (boundary.Observed, error)
}

// Rollback is what the comparison asks for at the harm exit. Every field of it is
// what the rollback's own deploy record will name.
type Rollback struct {
	...
	// [deploy.SourceComparisonAtHarm] every time this package asks.
	Source string
	// RevertIntentID is the intent the comparison raised before it asked, which is
...
var (
	// ErrWatchingIncomplete is returned for a service the comparison was not told
	// enough about.
	ErrWatchingIncomplete = errors.New("comparison: the service is missing something every call needs")
	// ErrNoSignal is returned by [New] for a comparison with no signal to read. A
	// comparison that reads nothing measures nothing, and a window it opened would
	// never close except at its cap.
	ErrNoSignal = errors.New("comparison: a comparison with no signal to read measures nothing")
)

// Comparison is the health signal over one factory.
...
// New returns the comparison over pool, writing windows, incidents, and the revert
...
// cannot roll back still has to watch — the window closing clean or at the cap is
// most of what watching does — and a harm exit there closes the window, writes the
// incident, and raises the revert intent without a rollback, which is exactly what
// the design does where a harm exit finds no release to return to.
func New(pool *pgxpool.Pool, windows *window.Writer, incidents *incident.Writer,
	...
	signal Signal, rollbacker Rollbacker) (*Comparison, error) {
	if signal == nil {
