// Package comparison is the health signal: what the factory does after a deploy,
...
// incident at a crossing, calls for a rollback at the harm exit, and raises an
...
// agent's and this package reaches none. That leaves the comparison a reader of
...
// a straight deploy does not do, and on a substrate that moves a process rather
// than traffic every deploy is straight. So no control is ever started here and
...
// the cap counts as a target: a release that was never condemned is one the factory
// can return to, and requiring a clean close would leave a service too quiet to
// reach one with no target at all. Closing swept does not — nothing is left running
// a swept release's build.
//
// Master is linear, so a rollback undoes the condemned release and every release
// above it, up to K. The condemned one is named apart from the swept ones on the
// rollback's own deploy record, and the open window of each swept release is closed
// swept. A swept release whose window had already closed keeps the exit it closed
...
//
// # What closes a window, and what a stopped comparison costs
//
// Nothing but this package closes a window, which is what the design requires: the
// comparison is what evaluates every exit. So a comparison that stops running
// leaves windows open, fills K, and holds that service's production deploys — a wait
...
// both.
//
// What defines it: ../../end-goal/how-humans-do-it/08-operations.md#the-health-signal
// for the control, the fallback, and what the comparison is;
...
// for K, the rollback's target, and what a rollback sweeps;
