...
// window closed without condemning it — clean or at the cap — and false where
...
//
// It descends past harm, past swept, and past any window still open. Nothing
...
// rollback goes further back and undoes releases nothing condemned, up to K, which
...
func TargetBelow(ctx context.Context, pool *pgxpool.Pool, serviceID string, number int64) (release.Release, bool, error) {
	closed, err := window.ClosedWithoutHarm(ctx, pool, serviceID)
	if err != nil {
	...
	for _, w := range closed {
		...
		if err != nil {
			return release.Release{}, false, fmt.Errorf("comparison: reading the release window %s watched: %w", w.ID, err)
		}
		if r.Number >= number {
			continue
		}
