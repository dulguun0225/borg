...
type Reading struct {
	...
	// closed carries its exit. It is the zero window for [Comparison.AfterWindow],
	...
	// WhyNoRollback is why a harm exit rolled nothing back, and is empty otherwise:
...
func (c *Comparison) Watch(ctx context.Context, w Watching) ([]Reading, error) {
	if err := w.validate(); err != nil {
		return nil, err
	}
	open, err := window.AllOpen(ctx, c.pool, w.ID)
	if err != nil {
		return nil, err
	}
	...
	for _, win := range open {
		...
		reading, sweptNow, err := c.read(ctx, w, win)
...
func (c *Comparison) read(ctx context.Context, w Watching, win window.Window) (Reading, []string, error) {
	reading := Reading{Window: win}

	rel, err := release.Get(ctx, c.pool, win.ReleaseID)
	if err != nil {
	...
	// Neither, so the cap: the condition a window is meant to reach is a traffic
...
// it would return to.
func (c *Comparison) observe(ctx context.Context, w Watching, rel release.Release) (boundary.Observed, release.Release, bool, error) {
	baseline, has, err := TargetBelow(ctx, c.pool, w.ID, rel.Number)
	if err != nil {
	...
	observed, err := c.signal.Read(ctx, q)
	if err != nil {
		return boundary.Observed{}, baseline, has, fmt.Errorf(
			"comparison: reading the quantity for release %d of %s: %w", rel.Number, w.Name, err)
	}
	return observed, baseline, has, nil
}

// harm is the exit with no human in it: the incident, the window closed, the
...
// intent with no rollback behind it is a state the design already has, for a harm
// exit that found no release to return to.
func (c *Comparison) harm(ctx context.Context, w Watching, reading Reading) (Reading, []string, error) {
	deployed, err := deploy.Get(ctx, c.pool, reading.Window.DeployID)
	if err != nil {
		return reading, nil, err
	}
	raised, err := c.recordCrossing(ctx, w, reading.Release, deployed.ID, true)
	if err != nil {
		return reading, nil, err
	}
	...
	reading.Window, reading.Exit = closed, window.ExitHarm

	target, has, err := TargetBelow(ctx, c.pool, w.ID, reading.Release.Number)
	if err != nil {
		return reading, nil, err
	}
	switch {
	case !has:
		...
		// without harm, no control is running under it, and there is no earlier build
		...
		return reading, nil, c.report(ctx, reading)
	case c.rollbacker == nil:
		reading.WhyNoRollback = "this factory is composed with no deploy agent to perform one"
		return reading, nil, c.report(ctx, reading)
	}

	above, err := release.Above(ctx, c.pool, w.ID, reading.Release.Number)
	if err != nil {
		return reading, nil, err
	}
	...
	if err := c.rollbacker.RollBack(ctx, rollback); err != nil {
		return reading, nil, fmt.Errorf("comparison: rolling release %d of %s back to %d: %w",
	...
	// comparison simply stopped, because master is linear and the release was above
	// the target. A window already closed keeps the exit it closed at — a window
	// closes once — so only the open ones are closed here.
	var closedSwept []string
	for _, r := range above {
		win, found, err := window.ForRelease(ctx, c.pool, r.ID)
		if err != nil {
	...
	return reading, closedSwept, c.report(ctx, reading)
}
...
// new incident to carry: a harm exit raises a revert, and a crossing after the
// window closed raises the item that investigates it.
func (c *Comparison) recordCrossing(ctx context.Context, w Watching, rel release.Release,
	deployID string, revert bool) (raisedCrossing, error) {
	open, found, err := incident.Open(ctx, c.pool, w.ID, rel.ID)
	if err != nil {
		return raisedCrossing{}, err
	}
	if found {
		observed, err := c.incidents.Observe(ctx, open.ID)
		if err != nil {
			return raisedCrossing{}, err
		}
	...
	raised, err := c.intake.TakeIn(ctx, Actor, intent.SourceDetector, statement)
	if err != nil {
		return raisedCrossing{}, err
	}
	written, err := c.incidents.Raise(ctx, Actor, incident.Raising{
		EnvironmentID: w.EnvironmentID,
		ServiceID:     w.ID,
		ReleaseID:     rel.ID,
		DeployID:      deployID,
...
// RevertStatement is what the comparison writes as the revert intent's statement.
...
func RevertStatement(serviceName string, number int64) string {
	return fmt.Sprintf("Revert release %d of %s: its watch window closed at harm, and %s.",
...
func (c *Comparison) report(ctx context.Context, reading Reading) error {
	if c.notifier == nil {
		return nil
	}
	...
	_, err := c.notifier.Notify(ctx, notifier.Wait{
