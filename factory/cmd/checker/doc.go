// Command checker is the independent checker's own process: one pass reading what each
// production target runs and comparing it against that service's current release.
//
// It is a second binary because a checker the factory deployed would be inside
...
// Three subcommands:
//
//	checker pass -secrets <file>
//	checker show
//	checker clear <mismatch-id> -human <name>
//
// pass is the comparison. show prints every mismatch and the last check per
// target, which is what makes a stopped independent checker visible rather than
...
// checker and refused at Ops. The three subcommands are that section's
// comparison, its last check per target read so a stopped independent checker is
...
// checker's own doc.go is what defines the two records this command writes.
package main
