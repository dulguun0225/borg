...
// as cmd/factory/main_test.go's newPath does — and the reconciler's own, on a
// schema of its own with its own schema applied through reconciler.Apply,
// exactly as cmd/factory/watch_test.go's newReconcilerStore does. Neither
...
func newStores(t *testing.T) (context.Context, stores) {
	t.Helper()
	ctx := t.Context()
	return ctx, stores{factory: newFactoryStore(t, ctx), own: newReconcilerStore(t, ctx)}
}
...
// newReconcilerStore is the reconciler's own store for one test: a schema of
// its own, its own schema applied by its own applier, and nothing of the
// factory's in it — the way cmd/factory/watch_test.go's newReconcilerStore
// opens one.
...
// and a service, both written the way an owner and the cut would write them,
...
func TestAPassWhereTheTargetAgreesRaisesNoMismatch(t *testing.T) {
	...
	held, why, err := reconciler.NewStore(s.own).Mismatch(ctx, svc.ID)
	if err != nil || held {
		t.Errorf("Mismatch = %v %q, %v; a target running the recorded build raises none", held, why, err)
	}
	comparisons, err := reconciler.LastComparisons(ctx, s.own, svc.ID)
	if err != nil || len(comparisons) != 1 || !comparisons[0].Agreed {
		t.Errorf("LastComparisons = %+v, %v, want one comparison agreeing", comparisons, err)
	}
}
...
// report a human at the reconciler reads.
func TestAPassWhereTheTargetDisagreesRaisesAMismatchTheReportNames(t *testing.T) {
	...
	uncleared, err := reconciler.Uncleared(ctx, s.own, svc.ID)
...
func TestATargetThatErrorsOnReadRunningWritesAnUnreachedComparisonAndRaisesNoMismatch(t *testing.T) {
	ctx, s := newStores(t)
	dir := t.TempDir()
	env, svc, credential := setUp(ctx, t, s.factory, dir)
	...
	if len(comparisons) != 1 || comparisons[0].Reached || comparisons[0].Why != failure.Error() {
		t.Errorf("LastComparisons = %+v, want one unreached comparison naming %q", comparisons, failure)
	}
	held, _, err := reconciler.NewStore(s.own).Mismatch(ctx, svc.ID)
	if err != nil || held {
		t.Errorf("Mismatch = %v, %v; failing to reach a target is not a mismatch", held, err)
	}
}

func TestNoProductionEnvironmentIsNothingToReconcileAndWritesNothing(t *testing.T) {
	ctx, s := newStores(t)
	credential := secretref.MustNew("deploy.local")
	calls := 0
	...
	if err != nil || len(comparisons) != 0 {
		t.Errorf("LastComparisons = %+v, %v, want nothing written", comparisons, err)
	}
}

func TestNoServicesIsNothingToReconcileAndWritesNothing(t *testing.T) {
	ctx, s := newStores(t)
	dir := t.TempDir()
	credential := secretref.MustNew("deploy.local")
...
// checked rather than asserted in a comment: nothing the reconciler writes is
