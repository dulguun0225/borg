...
// reconciler resolves it and hands the reference across the seam; what sits behind
// the seam is what would read a value, and on this substrate nothing does.
...
func dispatch(args []string) error {
	if len(args) == 0 {
		return errors.New("reconciler: a subcommand is required — " + subcommands)
	}
	switch args[0] {
	case "pass":
	...
	default:
		return fmt.Errorf("reconciler: %q is none of %s", args[0], subcommands)
	}
}
...
// open opens both and applies the reconciler's own schema. The factory's is not
// applied here: a store the reconciler applied would be a store the reconciler
// writes, and it reads that one.
func open(ctx context.Context) (stores, func(), error) {
	factory, err := postgres.Open(ctx, postgres.URL())
	if err != nil {
		return stores{}, nil, err
	}
	own, err := reconciler.Open(ctx, reconciler.URL())
	if err != nil {
		factory.Close()
		return stores{}, nil, err
	}
	if err := reconciler.Apply(ctx, own); err != nil {
		factory.Close()
		own.Close()
		return stores{}, nil, err
	}
...
func passCommand(args []string) error {
	...
	if *secrets == "" {
		return errors.New("reconciler pass: -secrets is required")
	}
...
func pass(ctx context.Context, s stores, out io.Writer, credential secretref.Ref,
	...
	if !found {
		fmt.Fprintln(out, "The factory has no production environment record; there is nothing to reconcile")
		return nil
	}
	...
	if len(services) == 0 {
		fmt.Fprintln(out, "The factory has no services; there is nothing to reconcile")
		return nil
	}

	writer := reconciler.NewWriter(s.own)
	for _, svc := range services {
		...
		for _, target := range production.Targets {
			...
			if err != nil {
				...
				// otherwise hold every production deploy, which is why the last comparison
				// exists at all.
				p.Why = err.Error()
			} else {
				p.Reached = true
...
// reconciler reads in the other direction.
...
// excusedBuilds is every build an open watch window accounts for: the build of the
// release under watch, and — where a substrate keeps one — the build of the control
// that window's deploy record names. A build running beside the current release is a
// mismatch only where no open window names it, or the reconciler would page on every
// rollout it sees.
//
...
func report(out io.Writer, serviceName string, p reconciler.Pass, written reconciler.Recorded) {
	switch {
	case !p.Reached:
...
// showCommand prints the store: every mismatch, and the last comparison per
// service and target. The second is what says whether this process is still running.
func showCommand(args []string) error {
	if len(args) != 0 {
		return errors.New("reconciler show: no arguments")
	}
	ctx := context.Background()
	s, shut, err := open(ctx)
	if err != nil {
...
func show(ctx context.Context, s stores, out io.Writer) error {
	all, err := reconciler.All(ctx, s.own)
	if err != nil {
		return err
	}
	if len(all) == 0 {
	...
	comparisons, err := reconciler.LastComparisons(ctx, s.own, "")
	if err != nil {
		return err
	}
	if len(comparisons) == 0 {
		fmt.Fprintln(out, "No comparison has ever been recorded, so this reconciler has never run")
		return nil
	}
	for _, c := range comparisons {
		if !c.Reached {
			fmt.Fprintf(out, "last comparison of %s on %s at %s: unreached — %s\n",
				c.ServiceID, c.Target, c.At, c.Why)
			continue
		}
		fmt.Fprintf(out, "last comparison of %s on %s at %s: agreed=%t, running %q against recorded %q\n",
...
func clearCommand(args []string) error {
	...
	if flags.NArg() != 1 {
		return errors.New("reconciler clear: one argument, the mismatch id")
	}
	if *human == "" {
		return errors.New("reconciler clear: -human is required")
	}
	...
	cleared, err := reconciler.NewWriter(s.own).Clear(ctx, flags.Arg(0), *human)
	if err != nil {
		return err
	}
