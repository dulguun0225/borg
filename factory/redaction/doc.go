...
// owner performs it, [Key], [Writer] and [NewWriter], the tx-taking [Insert]
// package policy calls inside the transaction that appends the policy
// version, and [Reaching], the legal hold's refusal. read.go is [OverKind],
// [ForTarget], [ByErasureKey] and [Get]. db_test.go is the tests, every one
// of them against the database.
//
...
// record. The row lands first — a stop between the two leaves a row saying
// words were removed and no record saying they were, which is the event
// visibly owing rather than visibly done, and the row is what a restore
// replays. So the row cannot be keyed by this record's id, which does not
// exist yet: [Key] derives the key from the actor, the target, the reason and
// the spans, the caller appends the row under it, and [Insert] derives it
// again from the fields it stores. The key is unique in [DDL] and
// [ByErasureKey] reads by it, so the erasure performed again appends no
// second row and writes no second record.
//
// # Who may write what
//
