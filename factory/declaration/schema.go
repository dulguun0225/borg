...
//
// The unique constraint is on the declaration version and the whole of what the
// predicate asserts, so one version cannot introduce the same assertion twice —
// which a derivation that read one field through two paths would otherwise
// produce. It is not on the item: an item authors as many declaration versions as
// its stage was attempted, and each version's predicates stand beside the
// previous version's the way every artifact version does.
//
...
// column in the factory has. The catalog a declaration draws from grows with the
// rest of gate policy — an owner extends it and a pin adds to it — so a list in the
// store would be a second place that catalog lives and would refuse a kind an
// owner had added. What refuses a kind this factory cannot decide is the
// derivation, which is where the design puts it.
//
...
var DDL = []string{
	...
	`create index if not exists declaration_by_item on ` + Table + ` (item_id)`,
	`create index if not exists declaration_by_producer on ` + Table + ` (producer_service_id, interface_name, element)`,
}
