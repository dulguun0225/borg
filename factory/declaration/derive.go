...
// parse cannot see as a selector — an unprotected assumption, which is what a
// pinned predicate covers. A read it invents is a field name the mirror shares with
// some other type in the consumer's own code, since the resolution is syntactic and
// nothing here type-checks — which is what withdrawing a pin, or the producer's
...
// something a declaration can be derived from: it does not parse, or it holds no
// exported struct type or more than one.
//
// It is an error and not an empty declaration, for the reason contract's own
...
// it as no declaration would silently drop the protection the consumer was owed.
var ErrDerivation = errors.New("declaration: a mirror file the derivation cannot read")

// ErrNotInCatalog is returned for a tag word naming a kind of assertion that is
// not in the catalog in force. A consumer picks from the catalog and cannot invent
// a kind of assertion at declaration time, and this is that rule at the
// derivation.
var ErrNotInCatalog = errors.New("declaration: that kind of assertion is not in the predicate catalog in force")
...
// producer, the interface, the element, and the kind. catalog is the predicate
// catalog in force, which is what limits the kinds a declaration may draw from; a
// tag naming a kind outside it is [ErrNotInCatalog], and one naming a kind inside
// it that this factory has no decider for is [gatepolicy.ErrPredicateKindUnknown]
// — which is where a catalog wide enough to admit an undecidable assertion is
// actually refused.
//
...
func Derive(root string, catalog []string) ([]Draft, error) {
	entries, err := os.ReadDir(root)
	if err != nil {
		return nil, fmt.Errorf("declaration: reading the checkout at %s: %w", root, err)
	}
	...
	for _, key := range order {
		...
		for _, field := range m.fields {
			...
			kinds, err := field.declares(catalog)
			if err != nil {
				return nil, fmt.Errorf("declaration: %s of %s: %w", field.name, key, err)
			}
			for _, k := range kinds {
...
// declares is what one mirror field asserts: the read predicate, because the
// field is read, and one more per tag word. The read comes first, so a reader of a
// declaration sees the assumption the derivation found before the ones the build
// stated.
func (f mirrorField) declares(catalog []string) ([]assertion, error) {
	...
	for _, a := range asserted {
		if !slices.Contains(catalog, string(a.kind)) {
			return nil, fmt.Errorf("%w: %s", ErrNotInCatalog, a.kind)
		}
...
func mirrorFields(path string) ([]mirrorField, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("declaration: reading %s: %w", path, err)
	}
...
// the root of its repository. It is the whole of what makes a declaration derived
// rather than stated: a name here is a name the consumer's code reads, and a
// mirror field that never appears is a field the consumer does not read.
//
