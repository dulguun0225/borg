// Package declaration owns the predicate a consumer's build declares about one
// element of one contract it reads: the record, the five kinds this factory can
// decide, what deciding one means, and the derivation of a consumer's predicates
// out of its checkout.
//
// # A predicate, not prose
//
// A declaration has to be decidable by the factory against one observed exchange,
// and it is drawn from a catalog the factory owns rather than invented at
// declaration time. The five kinds are package gatepolicy's, because the catalog
// they are the unauthored value of is a parameter of gate policy and two packages
// would otherwise each hold a copy of the list. What is here is what each one
// means against something: [Predicate.AgainstForm] and
// [Predicate.AgainstExchange].
//
...
// declaration version introducing them, so the version and its predicates cannot
// disagree about what was declared. That is why [Insert] takes a [pgx.Tx] rather
// than a pool. It is the second record-to-record import in the factory, package
// artifact importing this one, and it is taken for the reason the first was: the
// alternative is two writers of one table.
//
...
//
// Its authority does not come from the derivation being right. A derived
// declaration is an artifact of the consumer's item, ratified at the gates that
// item already passes; ratified that way it can reject a producer's candidate, and
...
// record.
//
// What is not here is which release a predicate entered force at. No release
// exists when a declaration is written, and the merge queue writing one later would
// be a second writer, so that is reached through the release record naming the same
// item — and the range of releases whose declarations bind at a moment is a
// question about windows and numbers, which belongs to whatever reads both.
//
// What defines it: the predicate, the catalog, the derivation, its authority, and
// the two baselines are
// ../../end-goal/how-humans-do-it/07-contracts.md#what-a-consumer-declares; the
// declaration being checked against the candidate is
// ../../end-goal/how-humans-do-it/07-contracts.md#what-a-diff-cannot-see; and the
// declaration version being an artifact of the item is
