// The database tests of this package are in declaration_test rather than in
// declaration, because they open the pool through package postgres, which imports
// this one to apply its DDL, and because the one writer of these rows is the
// artifact store. deps.txt records the edges as "test declaration -> postgres
// contract gatepolicy".
//
...
// implementer is the actor a declaration version is written as: the stage that
// derived it from the build.
var implementer = record.Actor{Kind: record.KindComponent, Name: "agent.implementer"}

// by is who authored the version, which for a derived declaration is the model the
// implementation stage ran on.
...
func newStore(t *testing.T) (context.Context, *pgxpool.Pool, *artifact.Store) {
	...
	schema := "m5_dec_" + hex.EncodeToString(suffix[:])
...
// resolved: a declaration carries both the name the build gave and the id it
// resolved to, and the empty id is a real answer.
const (
...
func draft(element string, kind gatepolicy.PredicateKind, argument string) declaration.Draft {
...
// TestTheStoreWritesTheVersionAndItsPredicatesTogether: a declaration version is an
// artifact of the consumer's item and the predicates it introduces are written in
// the same call, so the two cannot disagree about what was declared.
func TestTheStoreWritesTheVersionAndItsPredicatesTogether(t *testing.T) {
	ctx, pool, store := newStore(t)

	itemID := record.NewID("it")
	version, written, err := store.SubmitDeclaration(ctx, implementer, by, itemID, theConsumer,
		"2 predicates derived from the build", []declaration.Draft{
	...
	if version.Kind != artifact.KindDeclaration || version.Version != 1 {
		t.Fatalf("the version is kind %q at %d, want a declaration at 1", version.Kind, version.Version)
	}
	if len(written) != 2 {
		t.Fatalf("the version introduced %d predicates, want two", len(written))
	}
	read, err := declaration.ForArtifact(ctx, pool, version.ID)
	if err != nil {
...
// TestAPredicateTheDeclarationCannotDecideRollsTheVersionBack: the version and its
// predicates commit together or not at all, which is what stops a declaration
// version existing that nothing can be decided against.
func TestAPredicateTheDeclarationCannotDecideRollsTheVersionBack(t *testing.T) {
	ctx, pool, store := newStore(t)

	itemID := record.NewID("it")
	if _, _, err := store.SubmitDeclaration(ctx, implementer, by, itemID, theConsumer, "one bad predicate",
		[]declaration.Draft{
			draft("Status", gatepolicy.PredicateRead, ""),
	...
	newest, found, err := artifact.NewestOfKind(ctx, pool, itemID, artifact.KindDeclaration)
	if err != nil {
	...
	if read, err := declaration.ForItems(ctx, pool, []string{itemID}); err != nil || len(read) != 0 {
		t.Fatalf("ForItems = %d rows, %v", len(read), err)
	}
}

// TestForItemsReadsTheNewestVersionOfEachItem: a stage attempted twice authors two
// declaration versions, and what the item declares is the later one — the same rule
// the artifact store's version chain already sets.
func TestForItemsReadsTheNewestVersionOfEachItem(t *testing.T) {
	ctx, pool, store := newStore(t)

	itemID := record.NewID("it")
	if _, _, err := store.SubmitDeclaration(ctx, implementer, by, itemID, theConsumer, "first derivation",
		[]declaration.Draft{
			draft("Status", gatepolicy.PredicateRead, ""),
			draft("Detail", gatepolicy.PredicateRead, ""),
		}); err != nil {
		t.Fatalf("the first SubmitDeclaration: %v", err)
	}
	...
	if _, _, err := store.SubmitDeclaration(ctx, implementer, by, itemID, theConsumer, "second derivation",
		[]declaration.Draft{draft("Status", gatepolicy.PredicateRead, "")}); err != nil {
		t.Fatalf("the second SubmitDeclaration: %v", err)
	}

	read, err := declaration.ForItems(ctx, pool, []string{itemID})
	if err != nil {
	...
	if named := declaration.NamingElement(read, theProducer, theInterface, "Detail"); len(named) != 0 {
...
func TestOneVersionCannotIntroduceTheSameAssertionTwice(t *testing.T) {
	...
	_, _, err := store.SubmitDeclaration(ctx, implementer, by, record.NewID("it"), theConsumer, "twice",
		[]declaration.Draft{
			draft("Status", gatepolicy.PredicateRead, ""),
			draft("Status", gatepolicy.PredicateRead, ""),
		})
...
func TestAConsumerDeclaringAgainstAnUnpublishedInterfaceCarriesTheNameAlone(t *testing.T) {
	...
	version, _, err := store.SubmitDeclaration(ctx, implementer, by, record.NewID("it"), theConsumer,
		"against nothing published", []declaration.Draft{unresolved})
	if err != nil {
		t.Fatalf("SubmitDeclaration: %v", err)
	}
	read, err := declaration.ForArtifact(ctx, pool, version.ID)
...
func TestAgainstProducerAndConsumerServicesEverReadTheGraph(t *testing.T) {
	...
	for _, consumer := range []string{theConsumer, "svc_other", theProducer} {
		if _, _, err := store.SubmitDeclaration(ctx, implementer, by, record.NewID("it"), consumer,
			"one predicate", []declaration.Draft{draft("Status", gatepolicy.PredicateRead, "")}); err != nil {
			t.Fatalf("SubmitDeclaration for %s: %v", consumer, err)
		}
	}
	against, err := declaration.AgainstProducer(ctx, pool, theProducer)
	if err != nil {
	...
	ever, err := declaration.ConsumerServicesEver(ctx, pool, theProducer)
	if err != nil {
	...
	if len(declaration.ItemsOf(against)) != 3 {
		t.Errorf("the items behind those predicates are %v", declaration.ItemsOf(against))
	}
}
...
func TestTheStoreRefusesAroundTheWriter(t *testing.T) {
	...
	insert := `insert into declaration (id, actor_kind, actor_name, at, item_id, service_id,
	...
	for _, refused := range []struct {
		...
		_, err := pool.Exec(ctx, insert, record.NewID(declaration.IDPrefix), record.Now(),
