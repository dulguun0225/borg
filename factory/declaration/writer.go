...
// Of is the consumer this declaration version belongs to: the item, the service
// that item changes, and the declaration version the predicates are introduced by.
type Of struct {
	ItemID     string
	ServiceID  string
	ArtifactID string
...
// Insert writes every predicate one declaration version introduces, inside tx.
// Its one caller is the artifact store — the rows are written in the same call
// that submits the declaration version, so the version and the predicates it
// introduces cannot disagree about what was declared. That is why this takes a
// [pgx.Tx] rather than a pool: it runs inside the store's submitting transaction,
// and either both records commit or neither does.
//
...
// and the query that reads declarations in force is what stops seeing it.
func Insert(ctx context.Context, tx pgx.Tx, actor record.Actor, of Of, drafts []Draft) ([]Predicate, error) {
	...
	for _, d := range drafts {
		...
		if _, err := tx.Exec(ctx, `insert into `+Table+`
			...
			return nil, fmt.Errorf("declaration: writing %s of item %s: %w", p.Describe(), of.ItemID, err)
		}
