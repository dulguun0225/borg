...
// catalog is the catalog in force in these tests: the five kinds the factory owns,
// which is the unauthored value and what an owner extends.
var catalog = gatepolicy.PredicateCatalogNames()
...
func TestDeriveDeclaresTheFieldsTheCodeReads(t *testing.T) {
	dir := checkout(t, map[string]string{
		declaration.FileName("producer", "health"): mirror,
		"main.go": reads,
	})
	drafts, err := declaration.Derive(dir, catalog)
	if err != nil {
...
func TestDeriveIsSilentAboutAReadItCannotSee(t *testing.T) {
	...
	// cases: an unprotected assumption, and what a pinned predicate is for.
	dir := checkout(t, map[string]string{
		declaration.FileName("producer", "health"): mirror,
...
func TestDeriveRefusesAKindOutsideTheCatalog(t *testing.T) {
	...
	// A catalog an owner narrowed to the read predicate alone is not a state gate
	// policy can reach — a pin may only extend it — but it is what this check is
	// about: a consumer picks from the catalog and cannot invent a kind.
	_, err := declaration.Derive(dir, []string{string(gatepolicy.PredicateRead)})
	if !errors.Is(err, declaration.ErrNotInCatalog) {
		t.Fatalf("deriving against a catalog of one kind returned %v, want ErrNotInCatalog", err)
	}
}

func TestDeriveRefusesAMirrorItCannotRead(t *testing.T) {
	dir := checkout(t, map[string]string{
		declaration.FileName("producer", "health"): "package main\n\ntype A struct{}\ntype B struct{}\n",
	})
	if _, err := declaration.Derive(dir, catalog); !errors.Is(err, declaration.ErrDerivation) {
...
func predicate(element string, kind gatepolicy.PredicateKind, argument string) declaration.Predicate {
...
func TestAgainstExchangeDecidesEveryKind(t *testing.T) {
	...
	for name, of := range map[string]struct {
		...
		"empty in one":                {predicate("Status", gatepolicy.PredicatePopulated, ""), []declaration.Document{{"Status": "ok"}, {"Status": ""}}, false},
...
func TestAPredicateRefusesAnArgumentOfTheWrongShape(t *testing.T) {
	for name, p := range map[string]declaration.Predicate{
