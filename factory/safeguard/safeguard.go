...
var (
	...
	// ErrNotFound is returned by package policy where a withdrawal names no
	// safeguard by that id.
...
// Insert writes one safeguard inside tx. It is a method of [Writer], the
// record's one writer, so no other holder of a transaction and a token can
// call it. Its one caller is package policy, which calls it inside the
// transaction that appends the policy version, so the safeguard and the
// version commit together or not at all.
//
// w.token fences this write the way every write transaction in the module
// does: tx is begun by the caller — the policy version's own transaction — so
// this is where the fence is called rather than at a BeginTx of this
// package's own.
//
// id is minted by package policy and passed in, because the policy version
// names the safeguard and the version is appended first: the record cannot mint
// the id the version already carries.
//
...
func (w *Writer) Insert(ctx context.Context, tx pgx.Tx, actor record.Actor, id string,
	parameter gatepolicy.Parameter, subject Subject, bound Bound, routing Routing) (Safeguard, error) {
