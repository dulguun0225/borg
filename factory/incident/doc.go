...
// [Statuses] and [Incident.Open], [Writer] and [NewWriter], and the reads [Get],
// [All], [ForService], and [Open]. An incident names the deploy and through it
// the release, the item, and the intent, so what caused one is answered by
// following those links; there is no field saying what caused it, because the
// record points at the deploy that was running when it appeared and not always
// at the one that caused it.
//
