...
// its service.
//
// inputManifestID is the input manifest the run that authored this version was
// handed, written onto the row. It is the last argument of every submission,
// and it is empty where the caller that dispatched the run wrote no manifest.
func (s *Store) SubmitSpec(ctx context.Context, actor record.Actor, by By, itemID, serviceID, content string,
	...
	submitted, err := insertVersion(ctx, tx, actor, by, chainKey{ItemID: itemID}, KindSpec, content, inputManifestID, shipped{})
	if err != nil {
		return Artifact{}, nil, nil, err
	}
...
func (s *Store) SubmitImplementation(ctx context.Context, actor record.Actor, by By, itemID, content, inputManifestID string) (Artifact, error) {
	...
	submitted, err := insertVersion(ctx, tx, actor, by, chainKey{ItemID: itemID}, KindImplementation, content, inputManifestID, shipped{})
	if err != nil {
		return Artifact{}, err
	}
	if err := tx.Commit(ctx); err != nil {
...
// SubmitPlan writes an implementation plan version — the same versioning as a
// spec, no criteria. The content is how the item will be built, as the planner
// wrote it, and it is what the human at the plan gate reads and may edit in
// place.
func (s *Store) SubmitPlan(ctx context.Context, actor record.Actor, by By, itemID, content, inputManifestID string) (Artifact, error) {
	return s.submitItemDocument(ctx, actor, by, KindImplementationPlan, itemID, content, inputManifestID)
}

// SubmitTasks writes a tasks version: the approved plan divided into the work
// an agent picks up, one task per line, versioned the same way.
func (s *Store) SubmitTasks(ctx context.Context, actor record.Actor, by By, itemID, content, inputManifestID string) (Artifact, error) {
	return s.submitItemDocument(ctx, actor, by, KindTasks, itemID, content, inputManifestID)
}

// submitItemDocument is the write the plan and the tasks share: one version of
// one item's chain, introducing no record of another table. It is not exported,
// so a caller names the kind by calling the method for it rather than passing
// one.
func (s *Store) submitItemDocument(ctx context.Context, actor record.Actor, by By,
	...
	submitted, err := insertVersion(ctx, tx, actor, by, chainKey{ItemID: itemID}, kind, content, inputManifestID, shipped{})
	if err != nil {
		return Artifact{}, err
	}
	if err := tx.Commit(ctx); err != nil {
...
// under. It is a struct and not two arguments so that the two cannot be
// swapped, both being strings underneath, and it is the zero value on every
// authored submission.
type shipped struct {
	EnteredBy      EnteredBy
...
func insertVersion(ctx context.Context, tx pgx.Tx, actor record.Actor, by By, key chainKey, kind Kind,
	...
	a := Artifact{
		...
		ShippedBundleIdentity: entry.BundleIdentity,
