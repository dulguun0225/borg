...
Sitting 1 is finished. It raised eleven questions and each is folded into the file that owns its subject. Five went to [_Intent into items_](../end-goal/how-humans-do-it/02-intent-into-items.md): who writes an intent and at which event, what the interview leaves on the intent, that the cut writes items before the decomposition gate decides over them, what says which stage an item is at, and what an item names beyond its service. One went to [_Environments_](../end-goal/how-humans-do-it/05-environments.md), a candidate being a name for an item and a build rather than a record. Five went to [_Releases_](../end-goal/how-humans-do-it/06-releases.md): who writes the release record and mints the number, which links that record names itself, and when a deploy record is written, what it is keyed by, and which reading of current release its readers take. A record the sitting did not plan for arrived with the first — the end-user report — and its row above now names it, because a record found while a sitting runs still needs a named writer and named readers before that sitting can end.

Every record the sitting names has a named writer and named readers, and none is written by two components with no seam between them, which is the end condition above. [_Open_](../end-goal/open.md) holds nothing. The eight sittings between this one and the substrate are taken in whatever order a sitting allows, and the next one taken is _Before the cut_: it defines the constraint, the design system, the cut decision, the area, and the project, each of which sitting 1 read or wrote without defining. They are there rather than here because that is where an unsettled question about the factory already goes, and it is one place to edit when one is answered rather than two.
