...
This file uses the design document's vocabulary throughout — item, gate, cut, intent, record, seam, and the rest. Every one of them is one line in [_Glossary_](../end-goal/glossary.md), linked to the section that defines it.
...
Every record has a named writer and named readers, and the four written by two components each declare a seam: the item and the service on the event of the write, the environment record on its kind, and the interview question on asking against answering. That is the end condition above, so the cut can run. What it applies is the constraints in _Constraints on the cut_ against the design as it now stands.
