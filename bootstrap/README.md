...
The interview is finished. All ten sittings are folded into the files that own their subjects, and it left [_Open_](../end-goal/open.md) empty. What is in there now is the twenty-seven questions the first review pass raised on 2026-08-16, none of them from the interview. The cut can still run — the end condition is that every record has a named writer and named readers, which the interview met — but four of the twenty-seven bear on whether the cut's input is right: what dispatches the interview and the cut, which have no role and no scope basis; what an area's declaration is made of, which decides what the cut writes on every item; whether the risk threshold that decides the decomposition gate is defined anywhere; and whether a service being one repository is compatible with this repository being one.
