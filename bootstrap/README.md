...
Sitting 1 is running. Eight of its questions are answered. Five are folded into [_Intent into items_](../end-goal/how-humans-do-it/02-intent-into-items.md): who writes an intent and at which event, what the interview leaves on the intent, that the cut writes items before the decomposition gate decides over them, what says which stage an item is at, and what an item names beyond its service. One is in [_Environments_](../end-goal/how-humans-do-it/05-environments.md), a candidate being a name for an item and a build rather than a record, and two in [_Releases_](../end-goal/how-humans-do-it/06-releases.md), where the merge queue writes the release record and mints the number, and writes it once. A record the sitting did not plan for arrived with the first — the end-user report — and its row above now names it, because a record found while a sitting runs still needs a named writer and named readers before that sitting can end.

Three questions are open, and they are in [_Open_](../end-goal/open.md) with what turns on each — all three about the deploy record, the next being when one is written and how many a rollout produces. They are there rather than here because that is where an unsettled question about the factory already goes, and it is one place to edit when one is answered rather than two.
