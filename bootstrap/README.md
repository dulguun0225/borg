...
This file uses the design document's vocabulary throughout — item, gate, cut, intent, seam, and the rest. Every one of them is one line in [_Glossary_](../end-goal/glossary.md), linked to the section that defines it.
...
`end-goal/` is the intent. It goes the way the factory takes one, performed by hand: the interview that refines it, the cut, then a spec, an implementation plan, tasks, and an implementation per item — [_the interview_](../end-goal/how-humans-do-it/02-intent-into-items.md#the-interview) being the state the intent is in rather than a stage of its own. The chain repeats rather than running once — every item takes it again — which is why each pass has to stay cheap to repeat rather than ceremonious.
...
| 2 | Before the cut | constraint, standing and per-intent · design system · interview question · the cut decision · declared item order · partial intent · area · project |
...
| 8 | The watch | control · health signal · watch window and its four exits · K · restore floor and a rollback's target · explicit health threshold |
...
The interview is finished. All ten sittings are folded into the files that own their subjects, and it left [_Open_](../end-goal/open.md) empty. It is empty again: the first review pass raised twenty-seven questions there on 2026-08-16, and the owner did not trust that run, so all twenty-seven were discarded rather than taken, carried, or refused. What the pass changed in the rules stays — commit `90e5538` fixed a disposition rule and two blind checks, both in the instruction files — and only its findings went. Nothing in `end-goal/` moved on account of any of them, so discarding them returns the tree to where it stood, and a finding that was real is one the next audit raises again.

The cut can still run: the end condition is that every record has a named writer and named readers, which the interview met. What tests the design against something other than its own rules is the audit the root [`CLAUDE.md`](../CLAUDE.md) describes — thirty readers, twenty-eight of them the disciplines the project spans — and it has not been run since it was written.

Sitting 1 raised eleven questions. Five went to [_Intent into items_](../end-goal/how-humans-do-it/02-intent-into-items.md): who writes an intent and at which event, what the interview leaves on the intent, that the cut writes items before the decomposition gate decides over them, what says which stage an item is at, and what an item names beyond its service. One went to [_Environments_](../end-goal/how-humans-do-it/05-environments.md), a candidate being a name for an item and a build rather than a record. Five went to [_Releases_](../end-goal/how-humans-do-it/06-releases.md): who writes the release record and mints the number, which links that record names itself, and when a deploy record is written, what it is keyed by, and which reading of current release its readers take.

The nine that followed settled twenty-six more records in the graph and two outside it, and refused a record to sixty things their rows named, each with what answers it instead — a field, a name for two records already there, a parameter, or a query over records that exist. The sittings named three components `end-goal/` had described without a name: **dispatch** writes the item's stage, its attempt counts, and what each stage spent, every stage reporting a transition to it; the **artifact store** is the one entrance every artifact version is written through, three things being able to author one; and the **notifier** delivers on all three channels rather than a component per channel. Three things no sitting row planned for arrived the way the end-user report did in sitting 1, whose row names it only because a record found while a sitting runs is added to that row — an environment's **targets**, which turned out to be a field rather than records; a **factory policy** record for the two parameters no customer record's scope reaches; and the reconciler's **last comparison**, without which a check that stops is indistinguishable from one that finds nothing.

Four questions were left for the owner because no pattern in `end-goal/` answered them, and all four are answered: a service is one repository with one long-lived branch and no repository holds two; the contract machinery reaches any consumer in the factory whichever project it is in, so an intent's items may be in more than one; the decision log may be truncated to a checkpoint the chain restarts at; and a pin may not suspend the factory's own rollback. The second reversed a recommendation that had been argued from an objection sitting 6 already answered, and it is what makes the intent's project the originating project rather than a boundary.

Every record has a named writer and named readers, and the three written by two components each declare a seam: the item on the event of the write, the service on the field, and the environment record on its kind. That is the end condition above, so the cut can run. What it applies is the constraints in _Constraints on the cut_ against the design as it now stands.
