...
Sitting 1 is running. Three of its questions are answered and folded into [_Intent into items_](../end-goal/how-humans-do-it/02-intent-into-items.md): who writes an intent and at which event, what the interview leaves on the intent, and that the cut writes items before the decomposition gate decides over them. A record the sitting did not plan for arrived with the first — the end-user report — and its row above now names it, because a record found while a sitting runs still needs a named writer and named readers before that sitting can end.

Eight questions are open, and they are in [_Open_](../end-goal/open.md) with what turns on each — the next one taken is what says which stage an item is at. They are there rather than here because that is where an unsettled question about the factory already goes, and it is one place to edit when one is answered rather than two.
