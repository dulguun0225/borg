...
Nine questions are open, and they are in [_Open_](../end-goal/open.md) with what turns on each — the next one taken is whether items are written before or after the decomposition gate. They are there rather than here because that is where an unsettled question about the factory already goes, and it is one place to edit when one is answered rather than two.
